...
impl OperationModeHandler for CyclicSyncTorque {
    ...
    async fn set_target(&self, sdo: &mut SdoClient<impl CanDriver>, node_id: u8, target: &ModeTarget) -> Result<(), CanOpenError> {
        let tq = match target {
            ...
            _ => return Err(CanOpenError::Protocol("CST mode requires Torque target".to_string())),
        };
        sdo.download(node_id, 0x6071, 0, &OdValue::Integer16(tq)).await
    }
    ...
    async fn read_actual(&self, sdo: &mut SdoClient<impl CanDriver>, node_id: u8) -> Result<ModeActual, CanOpenError> {
        match sdo.upload(node_id, 0x6077, 0).await? {
            ...
            other => Err(CanOpenError::Protocol(format!("Expected i16 for actual torque, got {:?}", other))),
        }
    }
}
