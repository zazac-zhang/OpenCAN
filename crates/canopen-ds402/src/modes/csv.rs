...
impl OperationModeHandler for CyclicSyncVelocity {
    ...
    async fn set_target(&self, sdo: &mut SdoClient<impl CanDriver>, node_id: u8, target: &ModeTarget) -> Result<(), CanOpenError> {
        let vel = match target {
            ...
            _ => return Err(CanOpenError::Protocol("CSV mode requires Velocity target".to_string())),
        };
        sdo.download(node_id, 0x60FF, 0, &OdValue::Integer32(vel)).await
    }
    ...
    async fn read_actual(&self, sdo: &mut SdoClient<impl CanDriver>, node_id: u8) -> Result<ModeActual, CanOpenError> {
        match sdo.upload(node_id, 0x606C, 0).await? {
            ...
            other => Err(CanOpenError::Protocol(format!("Expected i32 for actual velocity, got {:?}", other))),
        }
    }
}
