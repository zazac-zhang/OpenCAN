...
impl OperationModeHandler for HomingMode {
    fn mode_value(&self) -> i8 { 6 }

    async fn set_target(&self, _sdo: &mut SdoClient<impl CanDriver>, _node_id: u8, _target: &ModeTarget) -> Result<(), CanOpenError> {
    ...
    async fn read_actual(&self, sdo: &mut SdoClient<impl CanDriver>, node_id: u8) -> Result<ModeActual, CanOpenError> {
        match sdo.upload(node_id, 0x6064, 0).await? {
            ...
            other => Err(CanOpenError::Protocol(format!("Expected i32 for position, got {:?}", other))),
        }
    }
}
